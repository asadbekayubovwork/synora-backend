...
tar --exclude='.venv' --exclude='__pycache__' --exclude='*.db' --exclude='.env' \
    -czf /tmp/synora.tar.gz app alembic alembic.ini requirements.txt
...
ssh root@169.58.183.151 '
  ...
  cd /opt/synora-backend && sudo -u synora .venv/bin/alembic upgrade head
  systemctl restart synora-api'
```

Note what the archive now carries: **`alembic` and `alembic.ini` as well as
`app`**. A new top-level directory that is not in that list silently never
ships, which for migrations would mean the code arriving without the schema it
needs.

`cd` first, because alembic looks for its ini in the working directory, and
`sudo -u synora` because it reads `DATABASE_URL` out of the server's `.env`.
Migrating *before* the restart is safe only for additive migrations — which
these all are. One that drops or rewrites a column the running code still uses
needs the old process stopped first.
